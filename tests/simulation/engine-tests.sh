#!/bin/bash
# Actual engine speed ordering, with host sanitizers.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "$SCRIPT_DIR/../../scripts/env.sh"
ENGINE="$WORK_DIR/packages/micropolis-engine"
OUT="$(mktemp -d "${TMPDIR:-/tmp}/micropolis-speed-engine.XXXXXX")"
trap 'rm -rf "$OUT"' EXIT
SOURCES=()
...
"${CXX:-clang++}" -std=c++17 -g -fsanitize=address,undefined -fno-sanitize-recover=all \
    -I"$ENGINE/src" -I"$PROJECT_ROOT/src" "$SCRIPT_DIR/engine-test.cpp" \
    "${SOURCES[@]}" -o "$OUT/test"
